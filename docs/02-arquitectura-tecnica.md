...
**Implementado — auditoría H1 ejecutada con datos reales** (`configs/provincias.csv`, `configs/produccion_documentada.csv`, `src/ingestion/midagri_aggregation.py`, `scripts/compute_n4_n5.py`): las 80 provincias de la Tabla 3-4 se construyeron y verificaron contra los nombres reales de provincia del archivo MIDAGRI (columna `PROVINCIA`). Se detectó y corrigió una inconsistencia real en la clasificación territorial: Gran Chimú (La Libertad) está marcada en la tesis como "requiere verificación" pero, a diferencia de Carabaya/Sandia (Puno) y La Mar/Huanta (Ayacucho), **no** se cuenta dentro de las "preliminares de sierra" de su departamento — se agregó la categoría `ClasificacionTerritorial.PENDIENTE_VERIFICACION`, excluida de N2 igual que selva/costa, distinta de `TRANSICION` (incluida en N2). Con esta corrección, `N0=1520, N1=1463, N2=1273, N3=1178` coinciden exactamente con la Tabla 5 de la tesis para el periodo original de 19 campañas. `midagri_aggregation.aggregate_to_provincia_campana()` suma los registros mensuales del archivo real a celdas provincia-campaña (sumando los meses de cada campaña ya derivada) para calcular N4 real. `scripts/compute_n4_n5.py` procesa las 5 hojas del archivo completo (~4.3 millones de filas) filtrando quinua de los 8 departamentos antes de agregar, evitando cargar el dataset completo en memoria.
